...
# (The previous full overlay replaced the decode dispatch too and froze at
# the first kv_step boundary crossed mid-decode; see
# MSA_PREFILL_SPEEDUP_HANDOFF_2026-07-08.md.)
import os as _msa_os

from .msa import (
    _stat as _msa_stat,
    _stat_value as _msa_stat_value,
...
_MSA_PREFILL_MAX_DENSITY = 0.5
_MSA_PREFILL_BLOCKWISE_TOPK_MIN_KV_LEN = int(
    _msa_os.environ.get("MLX_M3_MSA_PREFILL_BLOCKWISE_TOPK_MIN_KV_LEN", "32768")
    or "32768"
)
_MSA_PREFILL_BLOCKWISE_TOPK_BLOCK_CHUNK = int(
    _msa_os.environ.get("MLX_M3_MSA_PREFILL_BLOCKWISE_TOPK_BLOCK_CHUNK", "2")
    or "2"
)
...
class MiniMaxAttention(nn.Module):
    ...
    def _can_use_msa_prefill_attention(
        ...
        """Eligibility gate for the MSA fused prefill (ThunderMLX overlay).

        Pure check, NO side effects — callers rely on that: an ineligible
        step must fall through to the untouched native indexer path, which
        then performs this step's single index-cache update."""
        ...
        reason = None
        if not _MSA_PREFILL_ENABLED:
            reason = "disabled"
        elif not mx.metal.is_available():
            reason = "metal_unavailable"
        elif self.sparse_score_type != "max":
            reason = "score_type"
        elif B != 1:
            reason = "batch"
        elif L <= 1:
            ...
            reason = f"mask:{type(mask).__name__}"
        elif self.index_heads != K:
            reason = "index_heads"
        elif H % K != 0:
            reason = "head_divisibility"
        elif D % 32 != 0:
            reason = "head_dim"
        elif num_blocks < self.sparse_topk_blocks:
            reason = "too_few_blocks"
        elif sparse_density >= _MSA_PREFILL_MAX_DENSITY:
            reason = "density"

        if reason is not None:
            _msa_stat("prefill_ineligible")
            _msa_stat_value("last_prefill_ineligible_reason", reason)
            return False
        _msa_stat("prefill_eligible")
        return True
    ...
    def _msa_prefill_attention(
        ...
        """Fused blockwise-sparse prefill (ThunderMLX overlay).

        Index projections replicate MiniMaxM3Indexer.__call__ verbatim
        (proj -> reshape -> norm -> transpose -> rope -> index-cache
        update), so the index-key stream is identical to the native path.
        Once past the gate this path OWNS the index-cache update and always
        returns an output; the native indexer is skipped for this step."""
        if not self._can_use_msa_prefill_attention(
            queries, keys, mask, q_positions
        ):
            return None
        _msa_stat("prefill_attempt")
        B, L, _ = x.shape
        ...
        if (
            ...
            _msa_stat("prefill_standard_topk")
    ...
    def __call__(
        ...
        if use_sparse_mask:
            ...
            # ThunderMLX MSA fused prefill fast path: multi-token steps only.
            # Decode (L==1) and every ineligible case skip this hook with no
            # side effects and run the native path below unchanged.
            if L > 1 and _MSA_PREFILL_ENABLED:
