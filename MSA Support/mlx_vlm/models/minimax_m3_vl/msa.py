...
def _parse_decode_mma_splits(value) -> int:
    try:
        splits = int(str(value).strip() or "4")
    except (TypeError, ValueError):
        splits = 4
    return splits if splits in (1, 2, 4, 8, 16) else 4


_DECODE_MMA_SPLITS = _parse_decode_mma_splits(
    os.environ.get("MLX_M3_DECODE_MMA_SPLITS", "4")
)


def set_decode_mma_splits(value) -> int:
    global _DECODE_MMA_SPLITS
    _DECODE_MMA_SPLITS = _parse_decode_mma_splits(value)
    return _DECODE_MMA_SPLITS
_KERNEL_STATS_ENABLED = os.environ.get(
...
_KERNEL_STATS = {
    ...
    "prefill_eligible": 0,
    "prefill_ineligible": 0,
    "prefill_standard_topk": 0,
    "prefill_blockwise_topk": 0,
    "msa_k1_scalar": 0,
    "msa_k1_simd": 0,
    "msa_k1_simd_packed": 0,
    "msa_k1_steel_mma": 0,
    "msa_k1_steel_mma_compact": 0,
    "msa_k1_steel_mma_bq64": 0,
    "msa_k1_steel_mma_bq64_compact": 0,
    "last_prefill_ineligible_reason": None,
    "last_msa_k1_impl": None,
    "decode_attention_calls": 0,
    "direct_decode_error": 0,
    "direct_decode_eval_all": 0,
    "direct_decode_eval_small": 0,
    "decode_mma_calls": 0,
    "decode_mma_ineligible": 0,
    "last_decode_impl": None,
    "decode_select_kernel_calls": 0,
    "compact_decode_calls": 0,
    "compact_decode_ineligible": 0,
    "compact_decode_sort_calls": 0,
    "compact_decode_sort_skipped": 0,
    "decode_topk_reuse_hits": 0,
    "decode_topk_reuse_misses": 0,
    "last_compact_decode_selected_len": None,
    "last_compact_decode_total_len": None,
    "last_compact_decode_density": None,
    "sdpa_fallback_calls": 0,
    "last_direct_decode_eval_mode": None,
    "last_topk_native_error": None,
    "native_topk_disabled_reason": None,
}
_NATIVE_MSA_TOPK_DISABLED_REASON = None


def _stat(name: str, amount: int = 1):
    if _KERNEL_STATS_ENABLED:
        current = _KERNEL_STATS.get(name, 0)
        if isinstance(current, int):
            _KERNEL_STATS[name] = current + int(amount)
        else:
            _KERNEL_STATS[name] = int(amount)


def _stat_value(name: str, value):
    if _KERNEL_STATS_ENABLED:
        _KERNEL_STATS[name] = value
...
def reset_kernel_stats():
    for key in list(_KERNEL_STATS):
        _KERNEL_STATS[key] = None if key.endswith("_reason") or key.startswith("last_") else 0
...
def _call_minimax_msa_topk(
    ...
    ext = getattr(_minimax_fast, "_ext", None)
    ext_fn = getattr(ext, "minimax_msa_topk", None)
    if callable(ext_fn):
        return ext_fn(
            idx_queries,
            idx_keys,
...
_MSA_K1_STEEL_PROLOGUE_COMPACT = r"""
            constexpr short BQ = M_BLOCK_SIZE;
            constexpr short BK = 128;
            constexpr short BD = 128;
            constexpr short WM = M_BLOCK_SIZE / 8;
            ...
            constexpr short THREADS = WM * WN * 32;
            uint work = threadgroup_position_in_grid.x;
            constexpr int FLAT_ROWS = H_KV * TOTAL_ROWS;
            if (work >= uint(g_row_ptr[FLAT_ROWS])) {
                return;
            }

            // g_row_ptr is the exclusive prefix sum of per-(head,row) group
            // counts, so the launched grid covers exactly the non-empty
            // groups. Recover (head, row, group) with an upper-bound binary
            // search: largest flat index whose group offset is <= work.
            int left = 0;
            int right = FLAT_ROWS - 1;
            for (int step = 0; step < 32; ++step) {
                if (left < right) {
                    int mid = (left + right + 1) >> 1;
                    if (uint(g_row_ptr[mid]) <= work) {
                        left = mid;
                    } else {
                        right = mid - 1;
                    }
            ...
            int flat_row = left;
            int group_id = int(work - uint(g_row_ptr[flat_row]));
            int hkv = flat_row / TOTAL_ROWS;
            int row = flat_row - hkv * TOTAL_ROWS;
            int row_base = hkv * (TOTAL_ROWS + 1);
            int row_start = row_ptr[row_base + row];
            int row_end = row_ptr[row_base + row + 1];
            int edge_base = row_start + group_id * Q_TOKENS_PER_GROUP;
...
def _make_msa_csr_k1_steel_mma_kernel(compact_grid: bool = False):
    header = _load_mlx_steel_attn_header()
    if header is None:
        return None

    header += _MSA_STEEL_OPS_HEADER

    if compact_grid:
        name = "minimax_m3_msa_csr_k1_steel_mma_compact"
        input_names = ["q", "k", "v", "row_ptr", "qsplit", "g_row_ptr", "scale"]
        prologue = _MSA_K1_STEEL_PROLOGUE_COMPACT
    else:
        name = "minimax_m3_msa_csr_k1_steel_mma"
        input_names = ["q", "k", "v", "row_ptr", "qsplit", "scale"]
        prologue = _MSA_K1_STEEL_PROLOGUE_FULL
...
_MSA_CSR_K1_STEEL_MMA_COMPACT = (
    _make_msa_csr_k1_steel_mma_kernel(compact_grid=True)
...
# Off by default: offline A/B showed the compacted grid is bitwise-identical
# but performance-neutral at 100k-200k and slower at ~30k, because empty
# threadgroups are cheap and K1 is occupancy/memory bound, not dispatch bound.
# The compact grid still wins when paired with the BQ64 tile at long context
# (see the long-context auto dispatch below) and it removes the 2^31 grid
# ceiling that otherwise limits chunk size at 262k+ total tokens.
_MSA_K1_COMPACT_GRID = _parse_bool_env("MLX_M3_MSA_K1_COMPACT_GRID", "0")

# Long-context K1 auto dispatch. Offline A/B (bitwise-identical outputs):
# mma8 full-grid wins while K/V stays cache-resident (~<64k tokens); the
# BQ64 tile + compact grid wins once K/V spills to DRAM: 1.17x at 98k,
# 1.18x at 200k, 1.36x at 344k per layer-call.
_MSA_K1_LONG_CONTEXT_AUTO = _parse_bool_env("MLX_M3_MSA_K1_LONG_CONTEXT_AUTO", "1")
_MSA_K1_LONG_CONTEXT_MIN_TOTAL_K = int(
    os.environ.get("MLX_M3_MSA_K1_LONG_CONTEXT_MIN_TOTAL_K", "65536") or "65536"
)


def set_k1_long_context_auto(value) -> bool:
    global _MSA_K1_LONG_CONTEXT_AUTO
    if isinstance(value, str):
        _MSA_K1_LONG_CONTEXT_AUTO = value.strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        ...
        _MSA_K1_LONG_CONTEXT_AUTO = bool(value)
    return _MSA_K1_LONG_CONTEXT_AUTO


def set_k1_long_context_min_total_k(value) -> int:
    global _MSA_K1_LONG_CONTEXT_MIN_TOTAL_K
    _MSA_K1_LONG_CONTEXT_MIN_TOTAL_K = max(0, int(value or 0))
    return _MSA_K1_LONG_CONTEXT_MIN_TOTAL_K


def set_k1_compact_grid(value) -> bool:
    global _MSA_K1_COMPACT_GRID
    if isinstance(value, str):
        _MSA_K1_COMPACT_GRID = value.strip().lower() not in {"0", "false", "no", "off"}
    else:
        _MSA_K1_COMPACT_GRID = bool(value)
    return _MSA_K1_COMPACT_GRID


def _make_msa_decode_b1_mma_kernel():
    """Fused MSA sparse-decode split kernel (one threadgroup per KV head x
    selected block).

    Treats the 16 GQA query heads of one KV head as the M dimension of a
    Steel MMA tile, computes S = Q K^T and O_partial = softmax(S) V for a
    single selected KV block, and emits per-block partials plus LSE for a
    log-sum-exp combine. This reads K/V blocks exactly once and replaces the
    gather -> compact K/V -> SDPA decode chain with two dispatches.
    """
    header = _load_mlx_steel_attn_header()
    if header is None:
        return None
    ...
    return mx.fast.metal_kernel(
        name="minimax_m3_msa_decode_b1_mma",
        input_names=["q", "k", "v", "topk_idx", "topk_valid", "params", "scale"],
        output_names=["o_partial", "lse_partial"],
        header=header,
        ensure_row_contiguous=False,
        source=r"""
            constexpr short BQ = QHEAD_PER_KV;
            constexpr short BK = BLOCK_SIZE / SPLITS;
            constexpr short WM = QHEAD_PER_KV / 8;
            constexpr short WN = 1;
            constexpr short THREADS = WM * WN * 32;
            uint work = threadgroup_position_in_grid.x;
            if (work >= H_KV * TOPK * SPLITS) {
                return;
            }

            const int TOTAL_K = params[0];
            const int Q_POS = params[1];
            const int64_t q_h_stride = q_strides[1];
            const int64_t q_d_stride = q_strides[3];
            const int64_t k_h_stride = k_strides[1];
            const int64_t k_t_stride = k_strides[2];
            const int64_t k_d_stride = k_strides[3];
            const int64_t v_h_stride = v_strides[1];
            const int64_t v_t_stride = v_strides[2];
            const int64_t v_d_stride = v_strides[3];

            int split = int(work % SPLITS);
            int slot = int((work / SPLITS) % TOPK);
            int hkv = int(work / (SPLITS * TOPK));
            int part = slot * SPLITS + split;
            int64_t meta_idx =
                int64_t(TOPK_HEADS == 1 ? 0 : hkv) * topk_idx_strides[1]
                + int64_t(slot) * topk_idx_strides[3];
            int64_t meta_valid_idx =
                int64_t(TOPK_HEADS == 1 ? 0 : hkv) * topk_valid_strides[1]
                + int64_t(slot) * topk_valid_strides[3];
            int block = topk_idx[meta_idx];
            bool block_ok = bool(topk_valid[meta_valid_idx]) && block >= 0;
            ...
            if (!block_ok) {
                for (int m = int(tid_linear); m < BQ; m += THREADS) {
                    int hq = hkv * QHEAD_PER_KV + m;
                    lse_partial[part * H_Q + hq] = -INFINITY;
                }
                return;
            }

            int block_start = block * BLOCK_SIZE + split * BK;
            const int64_t k_head_base = int64_t(hkv) * k_h_stride;
            const int64_t v_head_base = int64_t(hkv) * v_h_stride;
            ...
            constexpr int TQ = BQ / (kNWarps * kFragSize);
            constexpr int TK = BK / kFragSize;
            constexpr int TD = D / kFragSize;
            static_assert(TQ == 1, "MSA decode MMA expects one Q tile per warp.");
            ...
            for (short dd = 0; dd < TD; dd++) {
                ...
                for (short iq = 0; iq < TQ; iq++) {
                    ...
                    for (short jj = 0; jj < MMAFrag_acc_t::kElemCols; jj++) {
                        int d = dd * kFragSize + sn + jj;
                        Qtile.frag_at(iq, 0)[jj] = float(
                            q[int64_t(hq) * q_h_stride + int64_t(d) * q_d_stride]);
                    }
                }
                STEEL_PRAGMA_UNROLL
                for (short ik = 0; ik < TK; ik++) {
                    ...
                    for (short jj = 0; jj < MMAFrag_acc_t::kElemCols; jj++) {
                        ...
                        Ktile.frag_at(0, ik)[jj] = k_pos < TOTAL_K
                            ? float(k[k_head_base
                                      + int64_t(k_pos) * k_t_stride
                                      + int64_t(d) * k_d_stride])
                            : 0.0f;
                    }
                }
                simdgroup_barrier(mem_flags::mem_none);
            ...
            Stile.template row_reduce<MSA_MaxOp>(max_score);
            STEEL_PRAGMA_UNROLL
            for (short i = 0; i < kRowsPT; ++i) {
            ...
            Stile.template row_bin_op<MSA_Exp2SubOp>(max_score);
            Stile.template row_reduce<MSA_SumOp>(sum_score);

            constexpr float LN2 = 0.6931471805599453f;
            float lse_val[kRowsPT];
            STEEL_PRAGMA_UNROLL
            for (short i = 0; i < kRowsPT; ++i) {
                lse_val[i] = sum_score[i] > 0.0f
                ...
                for (short id = 0; id < TD; id++) {
                    ...
                    for (short ik = 0; ik < TK; ik++) {
                        ...
                        for (short jj = 0; jj < MMAFrag_acc_t::kElemCols; jj++) {
                            ...
                            Vtile.frag_at(0, 0)[jj] = k_pos < TOTAL_K
                                ? float(v[v_head_base
                                          + int64_t(k_pos) * v_t_stride
                                          + int64_t(d) * v_d_stride])
                                : 0.0f;
                        }
                        simdgroup_barrier(mem_flags::mem_none);
                        MMAFrag_acc_t::mma(
            ...
            for (short iq = 0; iq < TQ; iq++) {
                ...
                int hq = hkv * QHEAD_PER_KV + m;
                if (sn == 0) {
                    lse_partial[part * H_Q + hq] = lse_val[iq];
                }
                STEEL_PRAGMA_UNROLL
                for (short id = 0; id < TD; id++) {
                    int d_base = sn + id * kFragSize;
                    ...
                    for (short jj = 0; jj < MMAFrag_acc_t::kElemCols; jj++) {
                        int d = d_base + jj;
                        o_partial[(part * H_Q + hq) * D + d] =
                            T(Otile.frag_at(iq, id)[jj]);
                    }
                }
            }
        """,
...
_MSA_DECODE_B1_MMA = (
    _make_msa_decode_b1_mma_kernel() if mx.metal.is_available() else None
)


_MSA_DECODE_BLOCK_SCORES = mx.fast.metal_kernel(
    name="minimax_m3_msa_decode_block_scores",
    ...
    source=r"""
        ...
        int tgs_per_head = (NUM_BLOCKS + blocks_per_tg - 1) / blocks_per_tg;
        int h = int(tg) / tgs_per_head;
        int block_base = (int(tg) % tgs_per_head) * blocks_per_tg;
        if (h >= H_IDX) {
            return;
        }

        threadgroup float q_s[D_IDX];
        for (int d = int(tid); d < D_IDX; d += THREADS) {
            q_s[d] = float(idx_q[int64_t(h) * idx_q_strides[1]
        ...
        if (block < NUM_BLOCKS && k_pos < TOTAL_K) {
            int64_t k_base = int64_t(k_pos) * idx_k_strides[2];
            float acc = 0.0f;
            for (int d = 0; d < D_IDX; ++d) {
                acc += q_s[d] * float(idx_k[k_base + int64_t(d) * idx_k_strides[3]]);
            }
            score = acc == acc ? acc : -INFINITY;
        }

        // Per-simdgroup max, then combine simdgroups covering the same block.
        float smax = simd_max(score);
        constexpr int SIMDS = THREADS / 32;
        constexpr int SIMDS_PER_BLOCK = BLOCK_SIZE / 32;
        threadgroup float partial_s[SIMDS];
        if (lane == 0) {
            partial_s[sgid] = smax;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (tid < uint(blocks_per_tg)) {
            ...
            if (b < NUM_BLOCKS) {
                ...
                for (int s = 0; s < SIMDS_PER_BLOCK; ++s) {
                    best = max(best, partial_s[int(tid) * SIMDS_PER_BLOCK + s]);
                }
...
_MSA_DECODE_TOPK_SELECT_SMEM = mx.fast.metal_kernel(
    ...
    source=r"""
        const int NUM_BLOCKS = params[0];
        const int CUR_BLOCK = params[1];
        ...
        threadgroup float scores_s[SMEM_CAP];
        threadgroup float red_score_s[THREADS];
        threadgroup int red_idx_s[THREADS];
        ...
        for (int b = int(tid); b < NUM_BLOCKS; b += THREADS) {
            float s = block_scores[h * NUM_BLOCKS + b];
            if (s != s) {
                s = -INFINITY;
            }
            if (INIT_BLOCKS > 0 && b < INIT_BLOCKS) {
                s = 1.0e30f;
            }
            if (LOCAL_BLOCKS > 0 && b >= local_start && b <= CUR_BLOCK) {
                s = 1.0e29f;
            }
            scores_s[b] = s;
        }
        ...
        for (int slot = 0; slot < TOPK; ++slot) {
            ...
            for (int b = int(tid); b < NUM_BLOCKS; b += THREADS) {
                float s = scores_s[b];
                if (s > best_score || (s == best_score && b < best_idx)) {
                    best_score = s;
                    best_idx = b;
                }
            }
            red_score_s[tid] = best_score;
            red_idx_s[tid] = best_idx;
            threadgroup_barrier(mem_flags::mem_threadgroup);
            for (uint stride = THREADS / 2; stride > 0; stride >>= 1) {
                if (tid < stride) {
                    float other = red_score_s[tid + stride];
                    int other_idx = red_idx_s[tid + stride];
                    if (
                        other > red_score_s[tid]
                        || (other == red_score_s[tid] && other_idx < red_idx_s[tid])
                    ) {
                        red_score_s[tid] = other;
                        red_idx_s[tid] = other_idx;
                    }
                }
                threadgroup_barrier(mem_flags::mem_threadgroup);
            }
        ...
        if (tid == 0) {
            for (int i = 1; i < TOPK; ++i) {
                int key = selected_s[i];
                int j = i - 1;
                while (j >= 0 && selected_s[j] > key) {
                    selected_s[j + 1] = selected_s[j];
                    --j;
                }
                selected_s[j + 1] = key;
            }
            for (int i = 0; i < TOPK; ++i) {
                ...
                topk_idx[h * TOPK + i] = idx < NUM_BLOCKS ? idx : -1;
            }
        }
    """,
)
...
def msa_decode_select_topk(
    ...
    *,
    ...
    """Fused decode-step sparse block top-k selection.

    Computes per-block max index scores and selects the forced-init/local
    top-k in two static-pipeline dispatches, replacing the
    matmul/pad/reshape/max/argpartition/sort chain. Returns sorted-ascending
    ``[1, H_idx, 1, topk]`` int32 block indices (``-1`` padded), or ``None``
    when the shape is unsupported so callers can fall back.
    """
    if not mx.metal.is_available():
        return None
    if idx_queries.ndim != 4 or idx_keys.ndim != 4:
        return None
    B, h_idx, L, d_idx = idx_queries.shape
    if B != 1 or L != 1 or idx_keys.shape[0] != 1 or idx_keys.shape[1] != 1:
        return None
    total_k = idx_keys.shape[2]
    if idx_keys.shape[3] != d_idx or total_k <= 0:
        return None
    num_blocks = (total_k + int(block_size) - 1) // int(block_size)
    if num_blocks < topk or num_blocks > 4096:
        return None
    if int(block_size) not in (32, 64, 128, 256):
        return None
    ...
    topk_idx = _MSA_DECODE_TOPK_SELECT_SMEM(
        inputs=[block_scores, params_b],
        template=[
            ("H_IDX", h_idx),
            ("TOPK", int(topk)),
            ("INIT_BLOCKS", int(init_blocks)),
        ...
        threadgroup=(threads, 1, 1),
        output_shapes=[(1, h_idx, 1, int(topk))],
        output_dtypes=[mx.int32],
    )[0]
    return topk_idx


_MSA_DECODE_B1_COMBINE = mx.fast.metal_kernel(
    name="minimax_m3_msa_decode_b1_combine",
    input_names=["o_partial", "lse_partial"],
    output_names=["out"],
    source=r"""
        ...
        for (int s = 0; s < PARTS; ++s) {
            row_max = max(row_max, lse_partial[s * H_Q + hq]);
        }
        if (row_max == -INFINITY) {
            out[elem] = T(0);
            return;
        ...
        float acc = 0.0f;
        for (int s = 0; s < PARTS; ++s) {
            float lse = lse_partial[s * H_Q + hq];
            if (lse == -INFINITY) {
                continue;
            }
            float weight = metal::exp(lse - row_max);
            denom += weight;
            acc += weight * float(o_partial[(s * H_Q + hq) * D + d]);
        }
...
def _build_grouped_msa_topk_native(
    ...
    global _NATIVE_MSA_TOPK_DISABLED_REASON
    force_native = _NATIVE_MSA_TOPK_MODE in {"1", "true", "yes", "on"}
    force_fallback = _NATIVE_MSA_TOPK_MODE in {"0", "false", "no", "off"}
    if force_fallback:
        _stat_value("native_topk_disabled_reason", "disabled by MLX_MINIMAX_MSA_NATIVE_TOPK")
        return None
    if _NATIVE_MSA_TOPK_DISABLED_REASON and not force_native:
        _stat_value("native_topk_disabled_reason", _NATIVE_MSA_TOPK_DISABLED_REASON)
        return None
    if not _has_minimax_fast_symbol("minimax_msa_topk"):
        _stat("topk_ineligible")
        return None
    if not mx.metal.is_available():
        _stat("topk_ineligible")
        return None
    ...
    try:
        ...
        _stat("topk_native_error")
        _stat_value("last_topk_native_error", message)
        if isinstance(e, TypeError) or "incompatible function arguments" in str(e):
            _NATIVE_MSA_TOPK_DISABLED_REASON = (
                "native minimax_msa_topk rejected this runtime signature; "
                "using MLX Metal fallback"
            )
            _stat_value("native_topk_disabled_reason", _NATIVE_MSA_TOPK_DISABLED_REASON)
        if force_native:
            raise
...
def msa_sparse_attention_b1_from_csr(
    ...
    *,
    ...
    if use_steel_mma_k1:
        use_bq64 = k1_impl == "steel_mma_bq64"
        use_compact_grid = _MSA_K1_COMPACT_GRID
        if (
            k1_impl == "auto"
            and _MSA_K1_LONG_CONTEXT_AUTO
            and total_k >= _MSA_K1_LONG_CONTEXT_MIN_TOTAL_K
        ):
            use_bq64 = True
            use_compact_grid = True
        q_tokens_per_group = 4 if use_bq64 else 8
        ...
        if work_items * threadgroup_size > 2**31 - 1:
            # The full grid would overflow Metal's 32-bit thread count; the
            # compact grid launches only non-empty groups and cannot overflow.
            use_compact_grid = True
        k1_kernel = _MSA_CSR_K1_STEEL_MMA
        selected_k1_impl = "steel_mma_bq64" if use_bq64 else "steel_mma"
        use_compact_grid = (
            use_compact_grid and _MSA_CSR_K1_STEEL_MMA_COMPACT is not None
        )
    elif use_packed_k1:
        ...
        selected_k1_impl = "simd_packed"
    else:
        q_tokens_per_group = 1
        groups_per_row_cap = 0
        k1_kernel = _MSA_CSR_K1_SIMD if use_simd_k1 else _MSA_CSR_K1_SCALAR
        selected_k1_impl = "simd" if use_simd_k1 else "scalar"

    g_row_ptr = None
    if use_steel_mma_k1 and use_compact_grid:
        # Exclusive prefix sum of per-(head,row) group counts. Launching
        # exactly the non-empty groups avoids the ~99% empty-threadgroup
        # dispatch overhead at long context (total_rows grows with context
        # while useful groups stay ~ nnz / Q_TOKENS_PER_GROUP).
        row_lens = row_ptr[:, 1:] - row_ptr[:, :-1]
        group_counts = (row_lens + (q_tokens_per_group - 1)) // q_tokens_per_group
        flat_counts = group_counts.reshape(-1).astype(mx.int32)
        zero = mx.zeros((1,), dtype=mx.int32)
        g_row_ptr = mx.concatenate([zero, mx.cumsum(flat_counts)], axis=0)

    eval_targets = [row_ptr, qsplit]
    if split_counts is not None:
        eval_targets.append(split_counts_for_k2)
    if g_row_ptr is not None:
        eval_targets.append(g_row_ptr)
    mx.eval(*eval_targets)

    if g_row_ptr is not None:
        total_groups = int(g_row_ptr[-1].item())
        if total_groups > 0:
            work_items = total_groups
            k1_kernel = _MSA_CSR_K1_STEEL_MMA_COMPACT
            selected_k1_impl = f"{selected_k1_impl}_compact"
        else:
            g_row_ptr = None

    _stat(f"msa_k1_{selected_k1_impl}")
    _stat_value("last_msa_k1_impl", selected_k1_impl)
    ...
    if g_row_ptr is not None:
        k1_inputs = [q, k, v, row_ptr, qsplit, g_row_ptr, float(scale)]
    else:
...
def msa_sparse_decode_b1(
    ...
    *,
    ...
    if _DIRECT_DECODE_EVAL_MODE in {"all", "full", "1", "true", "yes", "on"}:
        _stat("direct_decode_eval_all")
        _stat_value("last_direct_decode_eval_mode", "all")
        mx.eval(q, k, v, topk_idx, topk_valid)
    else:
        _stat("direct_decode_eval_small")
        _stat_value("last_direct_decode_eval_mode", "small")
        mx.eval(q, topk_idx, topk_valid)
...
def msa_sparse_decode_b1_mma_eligible(
    h_q: int, h_kv: int, dim: int, block_size: int
) -> bool:
    return (
        _MSA_DECODE_B1_MMA is not None
        and dim == 128
        and int(block_size) == 128
        and h_kv > 0
        and h_q % h_kv == 0
        and (h_q // h_kv) in (8, 16, 32)
    )


def msa_sparse_decode_b1_mma(
    q: mx.array,
    k: mx.array,
    v: mx.array,
    topk_idx: mx.array,
    ...
    *,
    ...
    """Fused Steel-MMA MSA sparse decode for the B=1, one-token hot path.

    Split phase: one threadgroup per (KV head, selected block) computes MMA
    attention partials with LSE. Combine phase merges block partials with a
    log-sum-exp weighted sum. K/V blocks are read exactly once, with no
    compact K/V materialization and no token mask tensors.
    """
    _stat("decode_attention_calls")
    ...
    bt, topk_heads, lt, topk = topk_idx.shape
    if (bq, lq, bk, bv, bt, lt) != (1, 1, 1, 1, 1, 1):
        raise ValueError("msa_sparse_decode_b1_mma supports only B=1 and L=1.")
    if (v_h_kv, v_total_k, k_dim, v_dim) != (h_kv, total_k, dim, dim):
        raise ValueError("K/V shapes must match q head dimension.")
    if topk_valid.shape != topk_idx.shape:
        raise ValueError("topk_valid shape must match topk_idx.")
    ...
    _stat("decode_mma_calls")
    _stat_value("last_decode_impl", "mma")

    # The kernel reads strided views directly (no compaction copies) and takes
    # total_k/q_pos as runtime params so a single Metal pipeline serves every
    # decode step. There is deliberately no mx.eval here: the dispatch stays
    # inside the lazy graph like any other op.
