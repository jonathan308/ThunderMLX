...
def clear_mlx_cache(reason):
    """Best-effort cache release for ordinary signal/watchdog exits."""
    try:
...
def heartbeat_stop():
    ...
    global _tokens_seen, _progress_tokens_seen
    with _lock:
        _generation_active = False
        _generation_started_at = None
        _last_token_progress = None
    ...
    def _watch():
        while True:
            ...
            if progress_tokens_seen > 0:
                ...
                timeout = min(PREFILL_TIMEOUT, PREFILL_STALL_FATAL)
                stall_kind = "prefill/liveness (chunk-silence fatal)"
