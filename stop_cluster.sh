...
if [[ -n "$STRAGGLERS" ]]; then
  ...
  ssh -o BatchMode=yes -o ConnectTimeout=8 -i "${M3_SSH_KEY:-$HOME/.ssh/id_ed25519_thundermlx}" "${M3_PEER:-worker.local}"     "pkill -9 -f '$CLUSTER/run_with_watchdog.py' 2>/dev/null; true" 2>/dev/null || true
fi
